...
FOR %%x IN ("C:\Program Files (x86)\xampp\php\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=C:\Program Files (x86)\xampp\php\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=C:\Program Files (x86)\xampp\php"
...
IF "%PHP_PEAR_CACHE_DIR%"=="" SET "PHP_PEAR_CACHE_DIR=C:\Program Files (x86)\xampp\tmp"
GOTO :INSTALLED
...
GOTO END
:RUN
"%PHP_PEAR_PHP_BIN%" -C -d memory_limit="-1" -d output_buffering=1 -f "%PHP_PEAR_INSTALL_DIR%\pearcmd.php" -- %1 %2 %3 %4 %5 %6 %7 %8 %9
:END
ATTRIB -R %PHP_PEAR_BIN_DIR%\pear.bat >nul
ATTRIB -R %PHP_PEAR_BIN_DIR%\peardev.bat >nul
ATTRIB -R %PHP_PEAR_BIN_DIR%\pecl.bat >nul

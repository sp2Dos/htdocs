...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[C:\Program Files (x86)\xampp\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[C:\Program Files (x86)\xampp\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[C:\Program Files (x86)\xampp\apache\include];
my ${exec_prefix} = q[C:\Program Files (x86)\xampp\apache];
my ${datadir} = q[C:\Program Files (x86)\xampp\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[C:\Program Files (x86)\xampp\apache\lib];
...
my ${prefix} = q[C:\Program Files (x86)\xampp\apache];
